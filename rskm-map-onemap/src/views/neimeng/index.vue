...
import { config, mapbox } from "@/config/tileserver-neimeng";
import { api } from "@/config/api";
import { layers, specEchars } from "@/config/spec-neimeng";
...
// 地图类型
let machine = ref("mercator");//globe mercator
watch(machine, () => {
    spin.value = true;
});
...
const loadData = () => {
    ...
    for (let i in data.filter((dd) => dd.properties.code != null)) {
        let properties = data[i].properties;
        let vales = 5;
        let text = ``;
        const encoder = new TextEncoder();
        if (properties.shuidao) {
            text += "\n水      稻";
            for (let i = 0; i < (vales - encoder.encode(properties.shuidao).length); i++) {
                text += " ";
            }
            text += properties.shuidao;
        }
        if (properties.handixiaomai) {
            text += "\n旱地小麦";
            for (let i = 0; i < (vales - encoder.encode(properties.handixiaomai).length); i++) {
                text += " ";
            }
            text += properties.handixiaomai;
        }
        if (properties.shuidixiaomai) {
            text += "\n水地小麦";
            for (let i = 0; i < (vales - encoder.encode(properties.shuidixiaomai).length); i++) {
                text += " ";
            }
            text += properties.shuidixiaomai;
        }
        if (properties.handiyumi) {
            text += "\n旱地玉米";
            for (let i = 0; i < (vales - encoder.encode(properties.handiyumi).length); i++) {
                text += " ";
            }

            text += properties.handiyumi;
        }
        if (properties.shuidiyumi) {
            text += "\n水地玉米";
            for (let i = 0; i < (vales - encoder.encode(properties.shuidiyumi).length); i++) {
                text += " ";
            }
            text += properties.shuidiyumi;
        }

        if (properties.dadou) {
            text += "\n大      豆";
            for (let i = 0; i < (vales - encoder.encode(properties.dadou).length); i++) {
                text += " ";
            }
            text += properties.dadou;
        }


        list.push(`${properties.county_name}`);
    ...
    map.getLayer('POI_WORLD_4') && map.setLayoutProperty('POI_WORLD_4', 'text-field', city);


    let addList = [];
    let dataAddList = data.filter((dd) => dd.properties.code == null);
    let ll = [];
    for (let i in dataAddList) {

        let properties = dataAddList[i].properties;
        let vales = 5;
        let text = ``;
        const encoder = new TextEncoder();
        if (properties.shuidao) {
...
const addPoint = (ll, addList) => {
    ...
    map.addSource('points', {
        ...
        "data": {
            ...
            "features": ll.map((point) => ({
                ...
                "properties": {
                    "name": point.name,
                    "shuidao": point.shuidao,
                    "dadou": point.dadou,
                    "handixiaomai": point.handixiaomai,
                    "shuidixiaomai": point.shuidixiaomai,
                    "handiyumi": point.handiyumi,
                    "shuidiyumi": point.shuidiyumi
                }
            }))
        }
    });

    // let list = [...new Set(addList)];
    //console.log(list)

    let city = [
        'match',
        ['get', 'name'], ...addList, ["get", "name"]
    ];
    ...
    map.addLayer({
        ...
        layout: {
            ...
            //   "text-field": '水稻 {shuidao}\n 旱地小麦 {handixiaomai}\n 水地小麦 {shuidixiaomai}\n 旱地玉米 {handiyumi}\n 水地玉米 {shuidiyumi}\n 大豆 {大豆}\n{name}',
            "text-field": city,
            "text-size": 20,
            "text-line-height": 1.2,
            "icon-image": "5500", // 引用精灵图中的图标
            "icon-size": 2.5,// 根据需要调整图标大小
...
window.addEventListener('message', (event) => {
    ...
    let code = data.data[0].code.substring(0, 6);
