...
const loadTown = async (name) => {
    ...
    if (activeKey.value == 1) {
        goGeom(ps, window.map)
        goGeom(ps, window.mapp)
    } else {
        goGeomLine(ps, window.map)
        goGeomLine(ps, window.mapp)
    }
...
const goGeom = (data) => {
    ...
    window.map.addSource("adminGeom", source)
    window.mapp.addSource("adminGeom", source)
    ...
    if (header.value) {
        ...
        window.map.addLayer(textLayer);
        window.mapp.addLayer(textLayer);
    } else {
        let adminGeomOut = {
            id: "adminGeomOut",
            type: "line",
...
const loadCounty = async (name) => {
    ...
    if (features.length == 1) {
        window.map.fitBounds(turf.bbox(JSON.parse(features[0].json)), {
            padding: { top: 100, left: 600, right: 600, bottom: 100 },
            linear: true
        });
        window.mapp.fitBounds(turf.bbox(JSON.parse(features[0].json)), {
            padding: { top: 100, left: 600, right: 600, bottom: 100 },
            linear: true
        });
    } else {
...
const loadLayers = (filter = undefined, newMap) => {
    if (!filter) {
        loadCounty("'东阿县','济阳区','莱芜区','桓台县','高青县','海阳市','招远市','汶上县','冠县','无棣县'", newMap);
    } else {
        if (activeKey.value == 2) {
            loadCounty("'" + filter + "'", newMap);
        } else if (activeKey.value == 1) {
            loadCounty("'" + filter + "'", newMap);
            loadTown("'" + filter + "'", newMap);
        }
    }
 ...
 * @param filter 
 */
const loadLocalData = (filter = undefined) => {
    header.value = filter;
    //console.log(filter)
    ...
    if (activeKey.value == 1) {
        loadLayers(header.value, window.map)
        loadLayers(header.value, window.mapp)
    } else {
        loadLayerDk(window.map)
        loadLayerDk(window.mapp)

        if (!header.value) {
            fitCenter(window.map)
            fitCenter(window.mapp)
        } else {
            loadCountyFit(`'${header.value}'`, window.map)
            loadCountyFit(`'${header.value}'`, window.mapp)
            loadTown(`'${header.value}'`, newMap)
        }
 ...
 * 隐藏地块图形
 */
const removeLayerDk = (newMap) => {
    newMap.setLayoutProperty('rskm_pt', 'visibility', 'none');
    newMap.setLayoutProperty('rskm_pt_outline', 'visibility', 'none');
    newMap.setLayoutProperty('rskm_pt_name', 'visibility', 'none');
    newMap.setLayoutProperty('rskm_pt_name_1', 'visibility', 'none');
 ...
 * 地块图形
 */
const loadLayerDk = (newMap) => {
    newMap.setLayoutProperty('rskm_pt', 'visibility', 'visible');
    newMap.setLayoutProperty('rskm_pt_outline', 'visibility', 'visible');
    newMap.setLayoutProperty('rskm_pt_name', 'visibility', 'visible');
    newMap.setLayoutProperty('rskm_pt_name_1', 'visibility', 'visible');
...
const loadTitleDIV = () => {
    //mapboxgl-compare
    let doc = document.getElementsByClassName("mapboxgl-compare")
    console.log(doc[0])
...
<template>
    ...
    <!-- 地图 -->
    <div class="verification" id="comparison-container">
    ...
    <div class="page">
        ...
        <div style="position: absolute;left: 15px;top: 120px;">
            ...
            <!-- {{ treeLeftData[0].children }} -->
        </div>

        <div class="xSquareShow" v-show="false" @click="xSquareShow = true">
        ...
        <div class="left-card" style="padding:0 10px" v-show="false">
        ...
        <div class="right-card">
            ...
            <a-typography-title :level="3" style="display: flex;align-items: center;">
                <MapPinned :size="28"></MapPinned>&nbsp; {{ header ? header : "全部试点县"
                }}&nbsp;<small>(玉米2024年)</small>
            </a-typography-title>


            <a-segmented v-model:value="segmented" block :options="data" size="large" /> <br>

            <a-card size="small" title="" style="height: 100%;">


                <a-tabs v-model:activeKey="activeKeys" tabBarGutter="2" :centered="true" tabPosition="top">
                    <a-tab-pane key="1" tab="整体">

                        <!-- <a-empty /> -->


                        <div v-if="segmented == '区域'">
                    ...
                    <a-tab-pane key="2" tab="地区">
                        ...
                        <div v-if="segmented == '区域'">
                            <a-table :columns="columnsQqdq" :data-source="dataQqdq" :pagination="false"
                                bordered></a-table>
                            <a-button type="link" :size="size">查看详细信息</a-button>
                        </div>

                        <div v-else>
                            <a-table :columns="columnsDkdq" :data-source="dataDkdq" :pagination="false"
                                bordered></a-table>
                            <a-button type="link" :size="size">查看详细信息</a-button>
                        </div>
                    </a-tab-pane>
                    <a-tab-pane key="3" tab="机构">
                        <!-- <a-empty /> -->
                        <a-table :columns="columnsQqjg" :data-source="dataQqjg" :pagination="false" bordered></a-table>
                        <a-button type="link" :size="size">查看详细信息</a-button>
                    </a-tab-pane>
                </a-tabs>
            </a-card>

            <a-card size="small" title="" style="height: 99%;" v-show="false">
...
::v-deep .mapboxgl-compare .compare-swiper-vertical {
    background-color: transparent;
    box-shadow: none;
    top: 10%;
    border-radius: 2px;
...
::v-deep .mapboxgl-compare {
    ...
    border-left: 3px dashed rgba(246, 243, 243, 0.394);
    background-color: transparent;
