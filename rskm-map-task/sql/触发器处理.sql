...
BEGIN
    ...
    -- 执行逻辑几何验证
        ...
        UPDATE rskm_pt AS d SET geom = ST_MakeValid(geom) WHERE NOT ST_IsValid(geom) and d.gid = NEW.gid;
        
        -- 执行逻辑填充省
        UPDATE rskm_pt AS d 
        SET province = a.name
        FROM admin_2022_province AS a
        WHERE  (ST_Within(d.geom, a.geom) or (ST_Area(ST_Intersection(d.geom, a.geom)) / ST_Area(d.geom)) > 0.6)
        AND d.gid = NEW.gid;
        ...
        -- 执行逻辑填充几何平米
	    UPDATE rskm_pt AS d SET area_mi =ROUND(ST_Area(d.geom::geography)::numeric,2) WHERE  d.gid = NEW.gid;
        
        -- 执行逻辑填充亩
		UPDATE rskm_pt AS d SET area_mu =ROUND((area_mi/666.667)::numeric,2) WHERE d.gid = NEW.gid;
